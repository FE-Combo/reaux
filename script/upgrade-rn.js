...
function generate() {
    const packageBuffer = fs.readFileSync("./package/doRN/package.json");
    ...
    fs.writeFileSync("./package/doRN/package.json", JSON.stringify(packageJSON));
    fs.writeFileSync("./package.json", JSON.stringify(targetPackageJSON));

    spawn("prettier", ["--config", "config/prettier.json", "--write", "{package/doRN/package.json,package.json}"]);
}
